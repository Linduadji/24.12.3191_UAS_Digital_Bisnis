    ...
    <div class="max-w-md w-full bg-white text-slate-900 rounded-4x1 p-8 shadow-2xl">
        ...
        <form action="{{ route('login.post') }}" method="POST" class="space-y-6">
            ...
            <div class="mt-8 mb-6 flex items-center justify-center space-x-4">
                ...
                <span class="text-sm font-bold text-slate-400">ATAU</span>
                <span class="h-px w-full bg-slate-200"></span>
            </div>

            <!-- Tombol Login Google (hanya untuk user publik) -->
            <a href="{{ route('google.login') }}"
                class="w-full flex items-center justify-center gap-3 py-4 bg-white border-2 border-slate-100 text-slate-700 rounded-2xl font-bold text-lg shadow-sm hover:bg-slate-50 hover:border-slate-200 hover:shadow transition">
