                        ...
                        <div class="relative">
                            <div class="bg-slate-100 rounded-2xl overflow-hidden aspect-video flex items-center justify-center border border-slate-200">
                                @if($transaction->event->poster_path)
                                    <img src="{{ Storage::disk('public')->exists($transaction->event->poster_path) ? asset('storage/' . $transaction->event->poster_path) : 'https://placehold.co/200x200' }}" alt="{{ $transaction->event->title }}" class="w-full h-full object-cover">
                                @else
