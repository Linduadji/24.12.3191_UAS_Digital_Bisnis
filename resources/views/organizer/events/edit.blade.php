    ...
    <form action="{{ route('organizer.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            @if($event->poster_path)
                <p class="mt-3 text-sm text-slate-500">Poster saat ini: <span class="font-semibold">{{ basename($event->poster_path) }}</span></p>
            @endif
        ...
        <div class="pt-4 flex justify-end gap-4 border-t border-slate-100">
            ...
            <button type="submit" class="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition">Perbarui Event</button>
        </div>
    </form>
</div>
@endsection