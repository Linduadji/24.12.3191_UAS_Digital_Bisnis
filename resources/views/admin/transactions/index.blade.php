                ...
                <tbody class="divide-y border-t">
                    @forelse($transactions as $transaction)
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-8 py-6">
                                @php
                                    $statusStr = strtolower(trim($transaction->status));
                                @endphp

                                @if(str_contains($statusStr, 'success') || str_contains($statusStr, 'settlement') || str_contains($statusStr, 'capture'))
                                    <span class="px-3 py-1 bg-emerald-500 text-white rounded-lg text-xs font-bold uppercase shadow-sm">
                                        {{ $transaction->status }}
                                    </span>
                                @elseif(str_contains($statusStr, 'pending'))
